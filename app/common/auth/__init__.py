...
class KeycloakOpenIDAuthBackend(AuthenticationBackend):
    async def authenticate(self, conn: HTTPConnection) -> Optional[Tuple[AuthCredentials, BaseUser]]:
        ...
        if endpoint in ['/api/v1/users/docs', '/openapi.json', '/api/v1/users/', '/api/v1/users/health/live/',
                        '/api/v1/users/health/ready/']:
            return AuthCredentials(['unauthenticated']), UnauthenticatedUser()

        if endpoint.startswith('/api/v1/users/graphql'):
            return AuthCredentials(['unauthenticated']), UnauthenticatedUser()
